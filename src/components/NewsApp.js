...
function NewsApp() {
    ...
    //https://newsapi.org/v2/everything?q=premierleague&from=2023-03-05&sortBy=publishedAt&apiKey=e0f8f3eb51424a6cb1bba79036ddbc2a
    const [newsList, setnewsList] = useState([]);
    const [query, setquery] = useState('premierleague');

    const queryRef = useRef(null)

    const apiurl = `https://newsapi.org/v2/everything?q=${query}&from=2023-04-05&sortBy=publishedAt&apiKey=${apiKey}`
    useEffect(() => {
        fetchData();
    }, [query])
